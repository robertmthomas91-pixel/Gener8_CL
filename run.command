...
if [ -z "$GEMINI_API_KEY" ]; then
  echo ""; echo "No Gemini API key found yet."; echo "Get one at https://aistudio.google.com/apikey (billing must be enabled)."; echo ""
  printf "Paste your Gemini API key and press Enter (or Enter to start without it): "
  read -r KEY_INPUT
  if [ -n "$KEY_INPUT" ]; then printf 'GEMINI_API_KEY=%s\nELEVENLABS_API_KEY=\nADMIN_EMAIL=admin@gener8.app\nADMIN_PASSWORD=change-me\nDATA_DIR=./data\nRETENTION_DAYS=30\nPORT=3000\n' "$KEY_INPUT" > .env; export GEMINI_API_KEY="$KEY_INPUT"; echo "✓ Saved .env (add ELEVENLABS_API_KEY for VO/Music; set your admin email/password)."; else echo "⚠  Starting without a key."; fi
  echo ""
fi
